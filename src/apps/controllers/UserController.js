...
class UserController{
        ...
        if(old_password){
            ...
            if(!new_password || !confirm_new_password){
                return res.status(401).json({
                    error: "We need a new password and confirm password attributes"
                })
            }
            ...
            encryptedPassword = await bcryptjs.hash(new_password, 10)
        }
        await Users.update(
            {
                name: name || user.name,
    ...
    async userProfile(req, res){
        ...
        const {id, name, user_name, avatar, bio, gender} = user;
